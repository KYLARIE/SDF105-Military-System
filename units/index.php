...
<body>
    ...
    $commanders = [];
    try {
        $cmdStmt = $pdo->query("SELECT p.id, p.name, p.rank_id, r.rank_name 
                          FROM people p 
                          LEFT JOIN ranks r ON p.rank_id = r.id
                          ORDER BY r.rank_name DESC, p.name ASC");
        while ($cmd = $cmdStmt->fetch(PDO::FETCH_ASSOC)) {
            $rank = !empty($cmd['rank_name']) ? $cmd['rank_name'] . ' ' : '';
            $commanders[$cmd['id']] = $rank . $cmd['name'];
        }
    } catch (PDOException $e) {
    ...
    $allPersonnel = [];
    try {
        $personStmt = $pdo->query("SELECT p.id, p.name, p.unit_id, r.rank_name 
                             FROM people p 
                             LEFT JOIN ranks r ON p.rank_id = r.id
                             ORDER BY r.rank_name DESC, p.name ASC");
