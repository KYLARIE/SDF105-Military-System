        ...
        if (strtolower($fileInfo['extension']) === 'csv') {
            ...
            try {
                while (($data = fgetcsv($handle)) !== false) {
                    if (count($data) >= 3) { // At least profile picture, full name, age
                        $profileImageUrl = trim($data[0]);
                        $name = trim($data[1]);
                        $age = (int)trim($data[2]);
...
function downloadAndSaveFile($url, $targetDir)
{
    ...
    // Handle Google Drive shared link with "open?id="
    if (strpos($url, 'drive.google.com') !== false) {
        // Extract file ID from various formats
        if (preg_match('/\/file\/d\/([a-zA-Z0-9_-]+)/', $url, $matches) || preg_match('/id=([a-zA-Z0-9_-]+)/', $url, $matches)) {
            $fileId = $matches[1];
    ...
    $extension = isset($extMap[$mimeType]) ? $extMap[$mimeType] : 'bin';
